...
Anyterm::response_t Anyterm::process_request(const HttpRequest& request)
{
	...
	try {
		...
		if (action=="open") {
			...
			string p_command = params.get("command") ;
			int timeout = params.get_as<int>("timeout" , ANYTERM_TIMEOUT) ;
			if (p_command!="" ) {
				activityfactory = SubProcessFactory(p_command) ;
			}
		...
		if (action != "resize" && rhost != ses->host) {
			return text_resp("EConsole has been taken over by another IP!<br>Reload the application to get it back!") ;
		}
		...
		if (action=="rcv") {
			...
			//ses->term.resize(params.get_as<int>("rows",25),params.get_as<int>("cols",25)) ;
			ses->set_host(rhost) ;
			return text_resp(idstr) ;
		
		} else if (action == "st") {
			
			int time_out = params.get_as<int>("timeout") ;
			ses->set_time_out(time_out) ;
			return text_resp(idstr) ;
